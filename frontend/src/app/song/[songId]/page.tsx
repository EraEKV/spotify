...
import { Button } from "@/app/components/ui/button-song";
...
// {props: {songId}} : {props: {songId: string}} 
const SongPage:React.FC<songProps> = () => {
  ...
  useEffect(() => {
    const fethcSongs = async () => {  
      try {
        const response = await fetch(`http://localhost:8080/api/v1/songs/${songId}`, {
          method: 'GET',
          headers: {
        ...
        <div className="grid md:grid-cols-[1fr_2fr] gap-8 ">
          ...
          <div className="grid gap-8">
            <div className="grid gap-4">
              ...
              <Slider
                className="w-full [&>span:first-child]:h-1 [&>span:first-child]:bg-white/30 [&_[role=slider]]:bg-white [&_[role=slider]]:w-3 [&_[role=slider]]:h-3 [&_[role=slider]]:border-0 [&>span:first-child_span]:bg-white [&_[role=slider]:focus-visible]:ring-0 [&_[role=slider]:focus-visible]:ring-offset-0 [&_[role=slider]:focus-visible]:scale-105 [&_[role=slider]:focus-visible]:transition-transform mt-12"
                defaultValue={[40]}
              />
